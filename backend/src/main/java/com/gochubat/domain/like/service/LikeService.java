...
public class LikeService {
	...
	public LikeService(PostLikeRepository postLikeRepository, PostRepository postRepository, PointService pointService) {
	...
	public LikeToggleResponse toggle(BoardType type, Long postId, Long userId) {
		...
		if (existing.isPresent()) {
			...
			pointService.award(authorId, PointReason.LIKE_REVOKED);
			liked = false;
		} else {
			postLikeRepository.save(PostLike.create(postId, userId));
			pointService.award(authorId, PointReason.LIKE_RECEIVED);
			liked = true;
		}
