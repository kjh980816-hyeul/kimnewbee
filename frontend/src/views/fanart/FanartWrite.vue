...
async function onSubmit(): Promise<void> {
  ...
  try {
    const fanart = await createFanart({
      title: title.value,
...
<template>
  <main class="min-h-screen bg-paper text-ink p-8">
    ...
    <form class="space-y-4 max-w-2xl" @submit.prevent="onSubmit">
      <div>
        <label class="block text-sm text-ink-muted mb-1" for="fanart-title">제목</label>
        <input
          id="fanart-title"
          v-model="title"
          type="text"
      ...
      <div>
        <label class="block text-sm text-ink-muted mb-1" for="fanart-content">설명 (선택)</label>
        <textarea
          id="fanart-content"
          v-model="content"
          rows="6"
