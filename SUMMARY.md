...
| 2-9 | File 업로드 | UploadProperties yml + UploadService (UUID 파일명 + path traversal 가드 + mime/size 검증) + POST /api/files (auth) + UploadResourceConfig 정적 서빙 + Spring multipart 한도 정렬 + 6 통합 테스트. **VM 로컬 디스크 기본값**, 운영 정책(S3 등) 변경 시 yml + UploadService 교체. | 385f12b |
